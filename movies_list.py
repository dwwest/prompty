...
# For every page in the interval
    for page in range(1,num_pages+1):
        ...
        movie_containers = page_html.find_all('a', class_ = "ipc-title-link-wrapper")

        with open('ids.html', 'w+') as f:
            f.writelines([str(mov) for mov in movie_containers])

        # Scrape the ID 
        new_ids = set([re.search(r'tt(\d+)/',str(movie)).group(1) for movie in movie_containers])
        assert ids < new_ids
        ids |= new_ids
